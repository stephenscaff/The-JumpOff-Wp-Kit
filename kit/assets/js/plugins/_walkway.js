...
--Usage:
-Mast on loads
* Timeout is required because of the slight delay from img to svg script
-----------------------------------------------  */
$(function(){
	setTimeout(function(){
		var svgdraw=new Walkway({selector:".svg-draw",easing:"easeInOutCubic",duration:2900});
		svgdraw.draw();
		}, 90);
});
...
if ($(window).width() > 500) {
//Services blocks
$('.icon-svg').mouseover(function(){
    	drawIconIA();
    	function drawIconIA() {
    	  var drawIconIA=new Walkway({selector:".icon-ia",duration:2500});
    	  drawIconIA.draw();
    	}
});
}